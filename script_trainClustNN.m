...
for Nh1 = 100 %[20 50 100] %number of hidden units in layer
    ...
    for Nc = [1,10] %[1, 2, 5, 10] %number of clusters
        ...
        Wlim = [-.1, 0.1]; %clamp weights between Wlim
...
%         Yte = Yte(Xte_clust==singleCluster, :);
        ...
        [W, b, centroids, riskValid, niters] = ...
            trainClustNN(W_init, b_init, Xtr, Ytr, Xv, Yv, centroid_init, splits, eta, Wlim, binW, thres, batchSize, maxIters);
        toc
        fprintf('\n')
...
%         miscStr   = sprintf('W_[%g_%g]', -0.01, 0.01);
%         miscStrT  = sprintf('W\\in[%g %g]', -0.01, 0.01);
        miscStr   = sprintf('W[%g_%g]_deltaW{%g,%g}', Wlim(1), Wlim(2), -1, 1);
        miscStrT  = sprintf('\\DeltaW\\in\\{%g,%g\\}, W\\in[%g,%g]', -1, 1, Wlim(1), Wlim(2));        
